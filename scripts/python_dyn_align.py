...
def is_tau_leaf(tree):
    return is_leaf(tree) and (tree.label is None or (type(tree.label) is tuple and tree.label[0] is None))
...
def get_subsequence_dict(pt_node):
    ...
    if not type(pt_node.label) is tuple:
        raise ValueError("Subsequence Dict cannot be computed: pt_node is not a process tree with ids")
...
def dyn_align(pt_node, letters_dict, trace, subsequence_dict):
    ...
    if pt_node.operator is None:
    ...
    elif pt_node.operator in [Operator.SEQUENCE, Operator.XOR, Operator.PARALLEL, Operator.LOOP]:
        ...
        match pt_node.operator:
            case Operator.SEQUENCE:  # Concatenation
                costs = _dyn_align_sequence(pt_node, letters_dict, trace, subsequence_dict)
            ...
            case Operator.PARALLEL:  # Parallel execution 
                costs = _dyn_align_shuffle(pt_node, letters_dict, trace, subsequence_dict)
...
def get_ids_from_process_tree(pt_node):
    if is_leaf(pt_node):
        ...
        ids = [pt_node._get_label()[1]] + [pt_id for child in pt_node.children for pt_id in get_ids_from_process_tree(child)]
    # sort the ids
    return sorted(ids)
...
def get_segments_for_sequence(trace, list_of_letters):
    ...
    if number_of_children > 2:
        ...
        segments = [(0, n), (n, 0)]
        split_positions = [i for i in range(1, n) if trace[i] in right_letters and trace[i-1] not in right_letters]
...
def _dyn_align_sequence(pt_node, letters_dict, trace, subsequence_dict):
    ...
    if n == 0:
        return sum([dyn_align(child, letters_dict, tuple(), subsequence_dict) for child in children])
    ...
    old_pos = 0
    costs = 0
    if len(trace) > len(children) and\
                    trace[0] in letters_dict[children[0]._get_label()[1]]\
        ...
        for child in children:
            ...
            while pos < n and trace[pos] in letters_current_child: 
                pos += 1
            costs += dyn_align(child, letters_dict, trace[old_pos:pos], subsequence_dict)
            old_pos = pos
    ...
    if number_of_children == 2:
        segments = get_segments_for_sequence(trace, [letters_dict[child._get_label()[1]] for child in children])
        costs = np.inf
        for partition in segments:
            split = partition[0]
            left_cost = dyn_align(children[0], letters_dict, trace[:split], subsequence_dict)
            if left_cost > upper_bound or left_cost > costs:
                continue
            right_cost = dyn_align(children[1], letters_dict, trace[split:], subsequence_dict)
    ...
    # as [0,a], [a,b], [b,c], ..., 
    vertices = [(i, j) for i in range(number_of_children+1) for j in range(n+1) if ((i > 0 and i < number_of_children) or (i == 0 and j == 0) or (i == number_of_children and j == n))]
    ...
    def outgoing_edges(v):
        ...
        for k in range(j, n+1):
            ...
            if k < n-1 and trace[k] in letters_dict[children[i]._get_label()[1]]:
                continue
            tmp_costs = dyn_align(children[i], letters_dict, trace[j:k], subsequence_dict)
    ...
    return min(upper_bound, costs[end])
...
def _dyn_align_shuffle(pt_node, letters_dict, trace, subsequence_dict):
    ...
    subtraces = [tuple([x for x in trace if x in letters_dict[child._get_label()[1]]]) for child in children]
    costs = [dyn_align(child, letters_dict, subtrace, subsequence_dict) for child, subtrace in zip(children, subtraces)]
...
def _dyn_align_loop(pt_node, letters_dict, trace, subsequence_dict):
    ...
    for (i, j) in segments:
        if i == j:
            ...
            for k in get_segments_for_sequence(trace[i:j], [q_letters, r_letters]):
                q_cost = dyn_align(q, letters_dict, trace[i:(i + k[0])], subsequence_dict)
                if q_cost > upper_bound or q_cost > costs:
                    continue
                r_cost = dyn_align(r, letters_dict, trace[(i + k[0]):j], subsequence_dict)
    ...
    for _ in range(n):
        ...
        for (i, j) in segments:
            ...
            new = min([cost_qr_segments[(i, k)] + cost_qr_segments[(k, j)] for k in range(i, j+1)], default=empty_trace)
            if new < old:
                change = True
    ...
    cost_r_segments = {i: dyn_align(r, letters_dict, trace[:i], subsequence_dict)
                       for i in range(n + 1) if cost_qr_segments[(i, n)] <= upper_bound}
    # Now, compute the costs for the loop
    costs = min([cost_r_segments[i] + cost_qr_segments[(i, n)] for i in cost_r_segments])
    return costs
